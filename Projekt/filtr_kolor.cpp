...
filtr_kolor::filtr_kolor()
{
}

void filtr_kolor::filtrowanie(Wlasciciele *wlasciciel, Samochody *samochod, Relacje *relacja, string nazwa_pliku)
{
	string kolor;
	cout << "Po jakim kolorze samochodu szukac wlascicieli: " << endl;
	cin >> kolor;
	int licznik_szukanych_samochodow = 0, ile_samochodow = wielkosc_tablicy_samochodow();
	for (int i = 0; i < ile_samochodow; i++)
	{
		if (kolor == samochod[i].get_kolor())
